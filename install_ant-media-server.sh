...
distro () {
  ...
  if [ -f "$os_release" ]; then
    . $os_release
    msg="We are supporting Ubuntu 20.04, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 26.04, CentOS Stream 9/10, RockyLinux 9/10 and AlmaLinux 9/10"
    if [ "$OTHER_DISTRO" == "true" ]; then
      ...
      if [[ $VERSION_ID != 20.04 ]] && [[ $VERSION_ID != 22.04 ]] && [[ $VERSION_ID != 24.04 ]] && [[ $VERSION_ID != 26.04 ]] && [[ $VERSION_ID != 10 ]] && [[ $VERSION_ID != 10.* ]] && [[ $VERSION_ID != 9* ]] && [[ $VERSION_ID != 12 ]] && [[ $VERSION_ID != 11 ]]; then
         echo $msg
         exit 1
            fi
    else
