...
distro () {
  ...
  if [ -f "$os_release" ]; then
    . $os_release
    msg="We are supporting Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 21.10 and Centos 8"
    if [ "$OTHER_DISTRO" == "true" ]; then
      ...
      if [ "$VERSION_ID" != "18.04" ] && [ "$VERSION_ID" != "20.04" ] && [ "$VERSION_ID" != "20.10" ] && [ "$VERSION_ID" != "21.10"] && [ "$VERSION_ID" != "8" ]; then
         echo $msg
         exit 1
            fi
    else
