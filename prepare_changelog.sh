...
get_change_log() 
{
  ...
  gh pr list --search "merged:>$DATE_OF_THE_TAG" -L 1000 --json title,body,number --template "{{range .}} <li> <a href='$PULL_REQUEST_BASE_URL/pull/{{.number}}'>{{.number}}</a> {{.title}} - <a href='{{.body}}'>{{.body}}</a></li>{{end}}" >> $CHANGE_LOG
